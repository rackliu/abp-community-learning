...
計算 / 練習題（中）
3. 設計一個高可用 Kubernetes 應用配置：最小 3 副本、自動伸縮、藍綠部署。（難度：中）
4. 計算不同資源限制下應用能支援的最大並發用戶數。（難度：中）

實作 / 編碼題（較難）
5. 編寫完整的容器化配置（Dockerfile、K8s YAML、Helm Chart）與 CI 流程。（難度：較難）
6. 實作自動伸縮、藍綠部署與故障自動恢復的 Kubernetes 配置。（難度：較難）
